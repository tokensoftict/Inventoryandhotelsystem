...
class TableInvoice extends Model
{
    ...
    public static function updateTableInvoice($request, $reports, $tableinvoice)
    {
        if ($tableinvoice != false) {

            //delete invoice payment

            $tableinvoice->invoice_items()->delete();

            $tableinvoice->invoice_item_batches()->delete();

            $payment_id = $tableinvoice->payment_id;

            $tableinvoice->payment_id = NULL;

            $tableinvoice->total_amount_paid = 0;
        ...
        if (config('app.store') == "inventory") {
            ...
            if ($tableinvoice->invoice_paper_number != $request->get('invoice_paper_number')) {

                $tableinvoice_paper_number_count = Invoice::where('invoice_paper_number', $request->get('invoice_paper_number'))->count();

                if ($tableinvoice_paper_number_count > 0)
                    $tableinvoice_paper_number = $request->get('invoice_paper_number') . "-" . date('Y-m-d') . '-' . mt_rand();
                else
                    $tableinvoice_paper_number = $request->get('invoice_paper_number');
            } else {
                $tableinvoice_paper_number = $tableinvoice->invoice_paper_number;
            }
        } else {

            $tableinvoice_paper_number = time();
        }
        ...
        $tableinvoice->invoice_paper_number = $tableinvoice_paper_number;
        if (config('app.store') == "inventory") {
            $tableinvoice->customer_id = $request->get('customer_id');
        }
        $tableinvoice->status = $request->get('status');
        $tableinvoice->sub_total = $totals['total_invoice_total_selling'];
        $tableinvoice->total_amount_paid = 0;
        $tableinvoice->total_profit = $totals['total_invoice_total_profit'];
        $tableinvoice->total_cost = $totals['total_invoice_total_cost'];
        $tableinvoice->last_updated_by = auth()->id();
        ...
        $tableinvoice_items_data = self::prepareInvoiceItemData(
            $reports,
        ...
        $tableinvoice_items_batches_data = self::prepareInvoiceItemBatchesData(
            $reports,
            $request->get('customer_id'),
            $request->get('status'),
            $request->get('date'),
    ...
    public static function createInvoice($request, $reports)
    {
        ...
        $invoice_data = [
            ...
            'customertable_id' => $request->get('customertable_id'),
            'warehousestore_id' => getActiveStore()->id,
